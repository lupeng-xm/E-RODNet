...
def data_augment(input_tensor, label_tensor):
    dice = np.random.rand()
    if dice <= 0.5:
        # Reverse the input tensor in the time dimension
        input_tensor = torch.flip(input_tensor, dims=[2])
        # Reverse the label tensor in the time dimension
        label_tensor = torch.flip(label_tensor, dims=[2])

    # Randomly decide whether to perform horizontal flipping
    if dice <= 0.25:
    ...
    # Randomly decide whether to add Gaussian noise
        ...
        noise = torch.randn(input_tensor.shape) * np.std(input_tensor.detach().numpy()) * 0.1 \
                + np.mean(input_tensor.detach().numpy())
        input_tensor = input_tensor + noise

        return input_tensor, label_tensor
    # Randomly decide whether to add both Gaussian noise and horizontal flipping
    elif 0.5 < dice <= 0.75:
        noise = torch.randn(input_tensor.shape) * np.std(input_tensor.detach().numpy()) * 0.1 \
                + np.mean(input_tensor.detach().numpy())
        input_tensor = input_tensor + noise
...
if __name__ == "__main__":
    ...
    if model_cfg['type'] == 'CDC':
        ...
        from nets.E_RODNet import E_RODNet as RODNet
    else:
    ...
    if model_cfg['type'] == 'CDC':
        ...
        rodnet = RODNet(SFF_channels=model_cfg['mnet_cfg'], n_class=n_class_train).cuda()
    ...
    for epoch in range(epoch_start, n_epoch):
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            # Data augmentation
            data, confmap_gt = data_augment(data, confmap_gt)
